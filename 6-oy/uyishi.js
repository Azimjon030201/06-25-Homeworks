// shuni hammasini async va await orqali qilish kerak
// 4
...
uploadImage({ fileName: "photo.jpg" })
    .then(compressImage)
    .then(sendToServer)
    .then(result => console.log("rasm serverga yuklandi", result));
...
async function validatePrice(product) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
...
addProduct({ name: "laptop", price: 500 })
    .then(validatePrice)
    .then(saveToDatabase)
    .then(result => console.log("mahsulot bazaga saqlandi", result));
...
sendMessage({ text: "Salom! Bu test xabar." })
    .then(confirmMessage)
    .then(renderMessage)
    .then(result => console.log("Xabar jarayoni yakunlandi:", result));
